...
class XRDViewer(QMainWindow):
    ...
    def _create_menus(self):
        ...
        legend_menu.addAction("Konum", self.set_legend_location)
    ...
    def set_legend_location(self):
        opts = ['best','upper right','upper left','lower left','lower right','right','center left','center right','lower center','upper center','center']
        loc, ok = QInputDialog.getItem(self, "Legend Konumu", "Seç:", opts, 0, False)
        if ok:
            leg = self.ax.get_legend()
            if leg:
                leg.set_location(loc)
                self.canvas.draw()
